...
for (const item of wordsApi.words) {
    ...
    assert.equal(
        item.hint.includes(item.word),
        false,
        `ヒントに答えが含まれています: ${item.word}`
    );
    assert.equal(item.difficulty, wordsApi.difficultyFor(item.word));
    assert.ok(item.acceptedAnswers.includes(item.word));
}
...
const balanced = wordsApi.selectWords({
    difficulty: "normal",
    count: 5,
    recentWords: [],
    random: () => 0.25
});
assert.equal(new Set(balanced.map(item => item.category)).size, 5);

const dailyOne = wordsApi.selectDailyWord({
    dateKey: "2026-07-30",
    difficulty: "normal"
});
const dailyTwo = wordsApi.selectDailyWord({
    dateKey: "2026-07-30",
    difficulty: "normal"
});
assert.equal(dailyOne.word, dailyTwo.word);
assert.ok(wordsApi.words.find(item => item.word === "ぱいん")
    .acceptedAnswers.includes("ぱいなっぷる"));
